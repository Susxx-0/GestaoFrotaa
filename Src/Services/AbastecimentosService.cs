...
namespace GestoreDeFrotas.Services
{
    public class AbastecimentosService
    {
        ...
        public AbastecimentosService(AppDbContext context) => _context = context;

        public async Task<IEnumerable<Abastecimento>> ObterTodosAsync() =>
            await _context.Abastecimentos.Include(a => a.Veiculo).OrderByDescending(a => a.Data).ToListAsync();

        public async Task<IEnumerable<Abastecimento>> ObterPorVeiculoAsync(int veiculoId) =>
            await _context.Abastecimentos.Where(a => a.VeiculoId == veiculoId).OrderByDescending(a => a.Data).ToListAsync();

        public async Task<Abastecimento?> ObterPorIdAsync(int id) =>
            await _context.Abastecimentos.Include(a => a.Veiculo).FirstOrDefaultAsync(a => a.Id == id);
        ...
        public async Task<Abastecimento> AdicionarAsync(Abastecimento ab)
        {
            ...
            int limiteLitros = _limitesPorMarca.ContainsKey(veiculo.Marca) ? _limitesPorMarca[veiculo.Marca] : LimiteGeral;
            if (ab.Litros > limiteLitros) throw new Exception($"A marca {veiculo.Marca} permite no máximo {limiteLitros} litros por abastecimento.");
            ...
            if (ultimo != null)
            {
                if (ab.KmAtual < ultimo.KmAtual) throw new Exception("O KM atual não pode ser inferior ao do último abastecimento.");
            ...
            {
                ab.KmPorLitro = 0; ab.ConsumoMedio = 0;
            }
